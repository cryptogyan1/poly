...
fn selected_pair_config() -> Result<PairConfig> {
    ...
    let count = [use_btc_eth, use_btc_sol, use_btc_xrp].iter().filter(|&&v| v).count();
    if count != 1 {
        anyhow::bail!("Exactly one pair must be enabled: PAIR_BTC_ETH, PAIR_BTC_SOL, PAIR_BTC_XRP");
    }

    if use_btc_eth {
        return Ok(PairConfig { left_name: "BTC", left_prefix: "btc", right_name: "ETH", right_prefix: "eth" });
    }
    if use_btc_sol {
        return Ok(PairConfig { left_name: "BTC", left_prefix: "btc", right_name: "SOL", right_prefix: "sol" });
    }
    Ok(PairConfig { left_name: "BTC", left_prefix: "btc", right_name: "XRP", right_prefix: "xrp" })
}
...
async fn extract_token_ids(condition_id: &str) -> Result<(String, String)> {
    ...
    if tokens.len() < 2 {
        anyhow::bail!("CLOB market {} has only {} tokens", condition_id, tokens.len());
    }
...
async fn main() -> Result<()> {
    ...
    let clob = if execution_mode == "direct" {
        Some(Arc::new(
            ClobClient::new(
                &rpc_url, &private_key, &proxy_wallet,
                api_key.clone(), api_secret.clone(), api_passphrase.clone(),
            ).await?,
        ))
    } else { None };
    ...
    let executor = if execution_mode == "executor" {
        let url = std::env::var("EXECUTOR_URL").unwrap_or_else(|_| "http://127.0.0.1:8787".to_string());
    ...
    loop {
        ...
        info!("✅ {} market: {} | UP={} DOWN={}",
            pair_cfg.left_name,  left_market.slug,
            &btc_up_id[..16], &btc_down_id[..16]);
        info!("✅ {} market: {} | UP={} DOWN={}",
            pair_cfg.right_name, right_market.slug,
            &eth_up_id[..16], &eth_down_id[..16]);
...
async fn discover_markets(
    ...
    let left = discover_market(api, pair_cfg.left_name, pair_cfg.left_prefix, now, &mut seen).await?;
    seen.insert(left.condition_id.clone());
    let right = discover_market(api, pair_cfg.right_name, pair_cfg.right_prefix, now, &mut seen).await?;
