...
impl WsMarketMonitor {
    ...
    async fn build_snapshot(&self) -> Result<MarketSnapshot> {
        ...
        let eth_up   = self.token_price(&self.tokens.eth_up_id,   required_shares).await?;
        let eth_down = self.token_price(&self.tokens.eth_down_id, required_shares).await?;
        let btc_up   = self.token_price(&self.tokens.btc_up_id,   required_shares).await?;
        let btc_down = self.token_price(&self.tokens.btc_down_id, required_shares).await?;
    ...
    async fn token_price(&self, token_id: &str, min_size: Decimal) -> Result<TokenPrice> {
        let book = self
            ...
            .get(token_id)
            .await
            .ok_or_else(|| anyhow::anyhow!("no WS data yet for token {}", &token_id[..16.min(token_id.len())]))?;
...
impl MarketMonitor {
    ...
    where
        ...
        Fut: std::future::Future<Output = ()> + Send + 'static,
    {
        info!("🎬 REST monitor starting (polling every {:?})", self.check_interval);
    ...
    async fn fetch_token_top(
        ...
        match fetch_orderbook(&self.api, token_id).await {
            Ok(book) => {
                ...
                let best_bid      = book.best_bid().map(|(p, _)| Decimal::try_from(p).unwrap_or(Decimal::ZERO));
                let best_bid_size = book.best_bid().map(|(_, s)| Decimal::try_from(s).unwrap_or(Decimal::ZERO));

                let selected_ask  = book.cheapest_ask_with_min_size(required_shares).or_else(|| book.best_ask());
                let best_ask      = selected_ask.map(|(p, _)| Decimal::try_from(p).unwrap_or(Decimal::ZERO));
                let best_ask_size = selected_ask.map(|(_, s)| Decimal::try_from(s).unwrap_or(Decimal::ZERO));

                (best_bid, best_ask, best_bid_size, best_ask_size)
            }
            Err(e) => {
                warn!("⚠️  Failed to fetch {} {} prices: {}", market_name, outcome_name, e);
                (None, None, None, None)
            }
        }
    }
