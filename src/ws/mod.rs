...
async fn handle_message(txt: &str, cache: &PriceCache, tx: &broadcast::Sender<BookUpdate>) {
    ...
    let v: Value = match serde_json::from_str(txt) {
        Ok(v)  => v,
        Err(e) => {
            warn!("WS JSON parse error: {} | raw={}", e, &txt[..txt.len().min(120)]);
            return;
        }
    };

    // ══════════════════════════════════════════════════════════════════════════
    // ROOT CAUSE FIX: Polymarket sends initial book snapshots as a JSON ARRAY:
    //   [{"event_type":"book","asset_id":..., "bids":[...], "asks":[...]}, ...]
    //
    // If we call v.get("event_type") on an array Value it returns None and we
    // silently drop the entire snapshot — leaving the cache empty forever, so
    // every subsequent price_change hits cache.get() == None and is also skipped.
    //
    // Fix: detect array, iterate, process each element through handle_value().
    // ══════════════════════════════════════════════════════════════════════════
...
// Inner handler — processes a single JSON object with an "event_type" field.
// Called both directly (for plain object messages) and from the array unwrapper above.
async fn handle_value(v: &Value, cache: &PriceCache, tx: &broadcast::Sender<BookUpdate>) {
    ...
    match event_type {
        ...
        "price_change" => {
            ...
            let mut seen_assets: std::collections::HashMap<String, (Option<Decimal>, Option<Decimal>)>
                = std::collections::HashMap::new();

            for change in &ev.price_changes {
                let price = match Decimal::from_str(&change.price) { Ok(p) => p, Err(_) => continue };
                let size  = match Decimal::from_str(&change.size)  { Ok(s) => s, Err(_) => continue };
                ...
                if let Some(mut book) = cache.get(&change.asset_id).await {
                    // Update the existing book with the changed level
                    ...
                    // No book snapshot received yet — bootstrap the cache directly from the
                    // best_bid/best_ask fields that Polymarket includes in every price_change.
                    // This handles the race condition where price_change arrives before book.
                    let bids = best_bid
                        .map(|p| vec![(p, Decimal::ONE)])   // dummy size=1 — only price matters
                        .unwrap_or_default();
                    let asks = best_ask
                        .map(|p| vec![(p, Decimal::ONE)])
                        .unwrap_or_default();
                    cache.update(&change.asset_id, bids, asks).await;
                    debug!("📥 cache seeded from price_change best_bid/ask for {}…",
                        &change.asset_id[..change.asset_id.len().min(20)]);
                }
                // (if neither book nor best_bid/ask available, skip — book snapshot will arrive)
            }
        ...
        "market_resolved" => {
            let winning = v.get("winning_outcome").and_then(Value::as_str).unwrap_or("?");
            let winning_id = v.get("winning_asset_id").and_then(Value::as_str).unwrap_or("?");
...
fn apply_level_unknown_side(
    ...
    if bids.iter().any(|(p, _)| *p == price) { upsert_level(bids, price, size); return; }
    if asks.iter().any(|(p, _)| *p == price) { upsert_level(asks, price, size); return; }
    // New level — infer side from position vs current best bid
    let best_bid = bids.iter().map(|(p, _)| *p).max().unwrap_or(Decimal::ZERO);
    if price <= best_bid { bids.push((price, size)); } else { asks.push((price, size)); }
}
